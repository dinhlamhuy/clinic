    ...
    <div class="col-md-12 px-0" style="background-color: #edf2f4;">
        <div class="row ">
            <div class="col-md-12 pl-4 pr-4">
                <div class="row mt-2">
                    <div class="col-md-12">
                        <span class="admin_titile">Quản lý tỉnh / thành phố</span>
                    </div>
                    ...
                    <div class="col-md-12 pl-3 pr-1 " style="background-color: white;">
                        <div class="row pt-1">
                            <div class="col-md-4"  style="border-left: 1px solid #b1154a;border-right: 1px solid #b1154a;border-top: 1px solid #b1154a;">
                                <center>
                                    <h4 class="mt-2">Thêm tỉnh / thành phố mới</h4>
                                </center>
                                <form action="{{ url('/admin/add_city') }}" class="pt-1" method="post">
                                    @csrf
                                    <input type="text" class="form-control" name="tenthanhpho" placeholder="Nhập tên tỉnh / thành phố mới" required>
                                    <div class="row mt-2"> 
                                        <div class="col-md-12">
                                            <center>
                            ...
                            <div class="col-md-8">
                                <table id="dsquoctich" class="display pt-2" style="width:100%;">
                                    ...
                                    <tbody>
                                        @foreach ($dsthanhpho as $tpho)
                                            
                                        <tr>
                                            <td width="20%;" class="text-center">{{ 'TP'.sprintf('%02d', $tpho->tp_ma )}}</td>
                                            ...
                                            <td width="16%;" class="text-center">
                                                ...
                                                <button type="button" class="btn btn-info float-left ml-3" data-toggle="modal"
                                                    data-target="#editthanhpho{{ $tpho->tp_ma }}"><i class="fa fa-pencil-square-o"
                                                        aria-hidden="true"></i></button>

<!-- Modal edit thành phố -->
<div class="modal fade" id="editthanhpho{{ $tpho->tp_ma }}" data-backdrop="static" data-keyboard="false" tabindex="-1"
    aria-labelledby="staticBackdropLabel" aria-hidden="true">
    <div class="modal-dialog">
        <div class="modal-content">
            <div class="modal-header">
                <h4 class="modal-title" id="staticBackdropLabel">Thông tin thành phố</h4>
                <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                    <span aria-hidden="true">&times;</span>
                </button>
            </div>
            <div class="modal-body">
                <form action="{{ URL::to('/admin/edit_city') }}" method="post">
                    @csrf
                        <div class="form-group">
                                <label for="" class="col-form-label chu float-left">Mã thành phố:</label>
                                <input type="hidden" name="tp_ma"  value="{{ $tpho->tp_ma }}" >
                                <input type="text" class="form-control" disabled value="{{ 'TP'.sprintf('%02d',$tpho->tp_ma) }}">
                        </div>
                        <div class="form-group">
                            <label for="" class="col-form-label chu float-left">Tên tỉnh / thành phố:</label>
                            <input type="text" class="form-control" name="tp_ten" value="{{ $tpho->tp_ten }}" required>
                        </div>
                    ...
                    <button type="submit" class="btn btn-primary center"><i class="fa fa-floppy-o"
                        aria-hidden="true"></i>&ensp;Lưu</button>
                                                        ...
                                                        <form action="{{ URL::to('/admin/delete_city') }}" method="post">
                                                            @csrf
                                                        <input type="hidden" name="tp_ma" value="{{ $tpho->tp_ma }}">
...
$('#quanlythanhpho').addClass('active');
